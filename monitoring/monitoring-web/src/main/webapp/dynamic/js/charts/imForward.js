...
$(function() {
	 ...
	 * 页面加载初始化下拉菜单
	 */
	$.post("/im/initFisData", {}, function(data) {
...
function initChars(fisData, secData, thrData) {
	...
	var f5 = $("#f5Select").val();
	var server_type = $("#firsSelect").val();
	if (server_type == 'ImShowRmsService') {
		$("#imShowRmsService").show();
		$("#imShowMdsService").hide();
		$("#imShowMtsService").hide();
	} else if (server_type == 'ImShowMdsService') {
		$("#imShowRmsService").hide();
		$("#imShowMdsService").show();
		$("#imShowMtsService").hide();
	} else if (server_type == 'ImShowMtsService'||server_type == 'ImShowTasService') {
		$("#imShowRmsService").hide();
		$("#imShowMdsService").hide();
		$("#imShowMtsService").show();
	} else {
		$("#imShowRmsService").hide();
		$("#imShowMdsService").hide();
		$("#imShowMtsService").hide();
								...
								load : function() {
									...
									refreshIntervalId = setInterval(function() {
										$.post("/im/queryData", {
											...
											// 动态更新顶部数据
											$("#WorkThread").text(bToObj.WorkThread);
											$("#CPU").text(bToObj.CPU);
											$("#Disk").text(bToObj.Disk);
											$("#MEM").text(bToObj.MEM);
											$("#RunTime").text(bToObj.RunTime);
											$("#InputCmds").text(bToObj.InputCmds);
											$("#OutputCmds").text(bToObj.OutputCmds);
											$("#ClientConnectionSum").text(bToObj.ClientConnectionSum);
											$("#GroupCount").text(bToObj.GroupCount);
	...
	$.post("/mf/queryCurrentData", {
		...
		id : id
			...
			function(data) {
				var bToObj = JSON.parse(data);
				var strHtml = "<table >";
				strHtml += "<tr><td style='text-align: right'><b>工作线程:" + bToObj.WorkThread
						+ "</b></td></tr>";
				strHtml += "<tr><td style='text-align: right'><b>持续运行时间 秒:" + bToObj.RunTime
						+ "</b></td></tr>";
				...
				if (server_type == 'ImShowRmsService') {
					strHtml += "<tr><td style='text-align: right'><b>已收到命令包数:" + bToObj.InputCmds
							+ "</b></td></tr>";
					//已响应命令包数
					strHtml += "<tr><td style='text-align: right'><b>已响应命令包数:" + bToObj.OutputCmds
					+ "</b></td></tr>";
				} else if (server_type == 'ImShowMdsService') {
					//GroupCount
					strHtml += "<tr><td style='text-align: right'><b>群组数量:" + bToObj.GroupCount
					+ "</b></td></tr>";
				} else if (server_type == 'ImShowMtsService'||server_type == 'ImShowTasService') {
                  //ClientConnectionSum  客户端连接数量
					strHtml += "<tr><td style='text-align: right'><b>客户端连接数量:" + bToObj.ClientConnectionSum
					+ "</b></td></tr>";
				}
				strHtml += "</table>";
				$("#hideDiv").hide();
