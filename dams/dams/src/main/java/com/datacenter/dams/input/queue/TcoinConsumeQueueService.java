 ...
 * @author wll
 */
public class TcoinConsumeQueueService{
	
	public static final Logger logger = LogManager.getLogger(TcoinConsumeQueueService.class);
	...
	public static void receiveMessage(int sum ,String destinationName){
		...
		/* 判断redis队列数据是否已满 */
		logger.debug("[DAMS#TBConsume]判断storm计算使用redis队列是否已满.");
		boolean flag = false;
		try {
		...
		/* 建立连接 */
		logger.debug("[DAMS#TBConsume]execute从PC队列中读取数据.");
		...
		try {
			...
			if(consumer != null){
				...
				for(int i=0;i<sum;i++){
					...
					if(message != null){
						...
						logger.info("[TB消费入口数据记录]>>总数是 : "+sum+" , 数据标示数 : "+i+" , 数据是 : message="+data);
						if(Strings.isNullOrEmpty(data)){
							break;
						}
						TcoinConsumeSpendInfo spend = (TcoinConsumeSpendInfo) JsonUtil.getObjectFromJson(data, TcoinConsumeSpendInfo.class);
						/* 处理信息 */
						MessageHandlerCenter.getTcoinConsumeSpendCenter().handler(spend);
						/* 处理消息 */
						session.commit();
						Thread.sleep(10);
			...
			logger.error("[DAMS#TBConsume#ERROR]worker读取PC队列数据出错.",e);
		}finally{
			try {
				session.close();
				connection.close();
			} catch (JMSException e) {
				logger.error("[DAMS#TBConsume#ERROR]队列关闭连接出错!",e);
			}
		}
		logger.debug("[DAMS#TBConsume]execute从PC队列中读取数据完成.");
	}
