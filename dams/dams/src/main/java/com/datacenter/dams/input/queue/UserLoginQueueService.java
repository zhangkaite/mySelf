 ...
 * @author wulinli
 */
public class UserLoginQueueService {

	private static Logger logger=LogManager.getLogger(UserLoginQueueService.class);
	private static JmsTemplate jmsTemplate;
	private static String userLoginDestination = "DAMS_UC_Login_Queue";
	...
	public static List<String> receiveMessage(int sum)throws Exception{
		if(sum != 0){
			...
			try {
				...
				if(consumer != null){
					...
					for(int i=0;i<sum;i++){
						...
						if(message != null){
							...
							logger.info("[DAMS用户登录记录UserLoginQueueService]UC队列数据出口>>总数是 : "+sum+" , 数据标示数 : "+i+" , 数据是 : message="+data);
				...
				logger.error("[DAMS用户登录记录UserLoginQueueService#ERROR]worker读取UC队列数据出错.",e);
			}finally{
				try {
					session.close();
					connection.close();
				} catch (JMSException e) {
					logger.error("[DAMS用户登录记录UserLoginQueueService#ERROR]队列关闭连接出错!",e);
				}
			}
		}
		return null;
	...
	public static void setJmsTemplate(JmsTemplate jmsTemplate) {
		UserLoginQueueService.jmsTemplate = jmsTemplate;
	}
	...
	public static void setUserLoginDestination(String userLoginDestination) {
		UserLoginQueueService.userLoginDestination = userLoginDestination;
	}
}
