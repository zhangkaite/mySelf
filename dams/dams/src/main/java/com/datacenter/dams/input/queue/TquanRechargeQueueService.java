 ...
 * @author wll
 */
public class TquanRechargeQueueService{
	
	public static final Logger logger = LogManager.getLogger(TquanRechargeQueueService.class);
	...
	public static void receiveMessage(int sum ,String destinationName){
		...
		try {
			...
			if(consumer != null){
				...
				for(int i=0;i<sum;i++){
					...
					if(message != null){
						...
						logger.info("[DAMS#TquanRechargeQueueService报表TQ充值数据]充值数据是："+data);
						if(Strings.isNullOrEmpty(data)){
							break;
						}
						TquanRechargeInfo spend = (TquanRechargeInfo) JsonUtil.getObjectFromJson(data, TquanRechargeInfo.class);
						/* 处理信息 */
						MessageHandlerCenter.getTquanRechargeServiceCenter().handler(spend);
						/* 处理消息 */
						session.commit();
						Thread.sleep(10);
			...
			logger.error("[DAMS#TquanRechargeQueueService报表TQ充值数据#ERROR]worker读取PC队列数据出错.",e);
		}finally{
			try {
				session.close();
				connection.close();
			} catch (JMSException e) {
				logger.error("[DAMS#TquanRechargeQueueService报表TQ充值数据#ERROR]队列关闭连接出错!",e);
			}
		}
	}
