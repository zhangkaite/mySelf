 ...
 * @author wll
 */
public class BrokerageRechargeQueueService{
	
	public static final Logger logger = LogManager.getLogger(BrokerageRechargeQueueService.class);
	...
	public static void receiveMessage(int sum ,String destinationName){
		...
		try {
			...
			if(consumer != null){
				...
				for(int i=0;i<sum;i++){
					...
					if(message != null){
						...
						logger.info("[DAMS#BrokerageRechargeQueueService兑换佣金数据]兑换数据是："+data);
						if(Strings.isNullOrEmpty(data)){
							break;
						}
						BrokerageRechargeInfo spend = (BrokerageRechargeInfo) JsonUtil.getObjectFromJson(data, BrokerageRechargeInfo.class);
						/* 处理信息 */
						MessageHandlerCenter.getBrokerageRechargeServiceCenter().handler(spend);
						/* 处理消息 */
						session.commit();
						Thread.sleep(10);
			...
			logger.error("[DAMS#BrokerageRechargeQueueService兑换佣金数据#ERROR]worker读取PC队列数据出错.",e);
		}finally{
			try {
				session.close();
				connection.close();
			} catch (JMSException e) {
				logger.error("[DAMS#BrokerageRechargeQueueService兑换佣金数据#ERROR]队列关闭连接出错!",e);
			}
		}
	}
