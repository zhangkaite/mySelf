...
public class TquanBillService implements BillInter{

	private static Logger logger=LogManager.getLogger(TquanBillService.class);
	...
	public void handler(Object object,String type) throws Exception {
		...
		try {
			...
			logger.info("[DAMS#TquanBillService]判断Hbase表是否已经存在,成功!");
		} catch (Exception e) {
			throw e;
		}
		...
		try {
			...
			logger.info("[DAMS#TquanBillService]添加TQ消费账单成功,数据是:"+data);
		} catch (Exception e) {
			logger.error("[DAMS#TquanBillService#ERROR]添加TQ消费账单失败,数据是:"+data+",",e);
			throw e;
		}
	}
	...
	private HbaseOperationInfo getHbaseOpertionInfoFromObject(Object object,String type){
		...
		if(type.equals("-1")){//消费
			TquanConsumeSpendInfo bill = (TquanConsumeSpendInfo)object;
			...
			info.setAccountType(new Integer(1));//TQ账户
			...
			info.setEquipID(bill.getEquipID() == null ? null : bill.getEquipID().toString());
			info.setUserType(new Integer(bill.getUserType()));
			return info;
		}
		if(type.equals("1")){//充值
			TquanRechargeInfo bill = (TquanRechargeInfo)object;
			HbaseOperationInfo info = new HbaseOperationInfo();
			info.setUserId(bill.getUserID());
			info.setTime(bill.getTime());
			info.setNumber(bill.getNumber());
			info.setTDNumber(bill.getTDNumber());
			info.setOrderId(bill.getOrderId());
			info.setClientType(bill.getClientType());
			info.setVersion(bill.getVersion());
			info.setTquanType(bill.getType());//充值类型
			info.setType(new Integer(type));//消费
			info.setAccountType(new Integer(1));//TQ账户
			return info;
		}
