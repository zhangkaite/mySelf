...
public class FisterConsumeStatistics extends AbstractBusStatisticsService {
	private static Logger logger = LogManager.getLogger(FisterConsumeStatistics.class);
	...
	public void doBusStatistics() {
		...
		try {
			...
			logger.error("读取hdfs上" + historyHdfsPath + "运营首消业务统计历史数据失败，失败的原因是：", e);
			return;
		}
		try {
			dataMap = readStatisticsHdfsData(statisticHdfsPath);
		} catch (Exception e) {
			logger.error("读取hdfs上" + statisticHdfsPath + "运营首消业务统计计算结果数据失败，失败的原因是：", e);
			return;
		}

		int consumeNum = dataMap.size();
		if (null != dataMap && dataMap.size() > 0) {
			...
			try {
				sendDataToMQ(queueName, dataMap, date, consumeNum);

			} catch (Exception e) {
				logger.error("运营首消业务统计结果推送dams MQ失败，失败的原因是:", e);
			}

			try {
				insertDataToHbase(dataMap, date, hbaseTableName);
			} catch (Exception e) {
				logger.error("运营首消业务统计结果写入hbase失败，失败的原因是:", e);
			}
			...
			try {
				...
				logger.error("运营首消业务统计结果数据写入hdfs路径" + targetHdfsPath + "失败，失败的原因是:", e);
			}
		}
	}
	...
	public Map readStatisticsHdfsData(String statisticHdfsPath) throws Exception {
		...
		Map resultMap = getHbaseStoreData(ls, ConsumeSpendConstant.FISTERCONSUME);
		return resultMap;
	}
	...
	public void sendDataToMQ(String queueName, Map resultData, String date, Integer num) throws Exception {
		...
		logger.info("运营首充消统计向SDMS ActiveMQ队列名称:" + queueName + "推送消息,内容:" + JsonUtil.getObjectToJson(postMap));
		// 追加 TB消费人数统计（去重）消息推送
		Map otherMap = new HashMap();
		otherMap.put("time", date);
		otherMap.put("size", String.valueOf(num));
		otherMap.put("type", "TB");
		SendActiveMqMessage(ConsumeSpendConstant.SDMS_ACTIVEMQ_TBCONSUME_SIZE_QUEUE,
				JsonUtil.getObjectToJson(otherMap));
		logger.info("运营TB消费总人数统计向SDMS ActiveMQ队列名称:" + ConsumeSpendConstant.SDMS_ACTIVEMQ_TBCONSUME_SIZE_QUEUE + "推送消息,"
				+ "内容:" + JsonUtil.getObjectToJson(otherMap));
	}
}
