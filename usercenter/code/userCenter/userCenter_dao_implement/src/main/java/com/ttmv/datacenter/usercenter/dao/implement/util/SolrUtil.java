...
public class SolrUtil {
	...
	/**
	 * 通过条件精确查询 还没添加：时间段 的查询
	 * nickName字段是模糊查询
	 * @return
	 * @throws SolrServerException
	 */
	public static QueryResponse assemblyConditionsOfSelect(SolrServer solrServer, Map<String, Object> fieldValues,Map<String,List<String>> timeMap , Integer start, Integer count,Map<String, Boolean> sortFields,List<String> showFields,SentryAgent quickSentry)throws Exception {
		...
		if(timeMap != null && timeMap.size() > 0){
			...
			while (iteratorTime.hasNext()) {
				...
				y++;
				if(y != timeMap.size() ){
					queryTime.append(field + ":[" + timeMap.get(field).get(0) + " TO " +  timeMap.get(field).get(1) +"] AND ");
				}else if(y== timeMap.size() ){
					queryTime.append(field + ":[" + timeMap.get(field).get(0) + " TO " +  timeMap.get(field).get(1) +"]");
				}
			}
		...
		try {
			...
			throw new Exception("solr查询失败",e);
		}
		return response;
	}
	...
	/**
	 * 通过唯一关键字查询信息：
	 * 只返回userid 和dataSourceKey
	 * @return
	 * @throws SolrServerException
	 */
	public static QueryResponse selectByKeyWordReturnUserIdAndDataSourcekey(SolrServer solrServer,
			String field, String key, int start, int count,SentryAgent quickSentry)
			throws Exception {
		SolrQuery query = null;
		/* 检查参数是否合法 */
	 ...
	 * @throws SolrServerException
	 */
	public static QueryResponse selectByKeyWord(SolrServer solrServer,
			String field, String key, int start, int count,SentryAgent quickSentry)
			throws Exception {
		SolrQuery query = null;
		/* 检查参数是否合法 */
	 ...
	 * @return
	 */
	public static QueryResponse selectByIds(SolrServer solrServer,List<BigInteger> ids,SentryAgent quickSentry)throws Exception{
		SolrQuery query = null;
		/* 检查参数是否合法 */
