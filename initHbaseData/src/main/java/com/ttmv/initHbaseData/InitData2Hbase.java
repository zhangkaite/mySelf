...
public class InitData2Hbase {
	...
	public static void initDatas2Hbase(String startTime,String endTime,String currentTime) throws Exception{
		...
		// 获取当前时间昨天的时间起始时间和结束时间
				try {
					...
					initStaticDatas2Hbase(currentTime,result_map);
	 ...
	 */
	public static void initStaticDatas2Hbase(String startTime,Map resulMap) throws ParseException {
		// 如果hbase表不存在，先创建表
		...
		for (Object obj : resulMap.keySet()) {
			...
			for (Object result : result_map.keySet()) {
				...
				String hbase_key = sdf.parse(startTime).getTime() + channel_id + user_id;
				...
				try {
					...
					System.out.println("插入hbase的数据rowkey:"+hbase_key+";json数据:"+value);
				} catch (Exception e) {
					// TODO 添加报警机制
