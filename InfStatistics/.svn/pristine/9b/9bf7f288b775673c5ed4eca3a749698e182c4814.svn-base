...
public class RedisUtil {
	...
	public static List<Object> getValueBatch(String key, int number) throws Exception {
		...
		String luaScript = " local list= {} " + " local num = redis.call('llen',KEYS[1])" + " if num == 0 then return nil end"
				+ " for i=1,ARGV[1],1 do " + "    local value = redis.call('lpop',KEYS[1]) " + "    if value then"
				+ "        list[i] = value " + "    else " + "        break" + "    end" + " end " + " if #list<1 then "
				+ "    return nil " + " end " + " return list";
	...
	public static void pushData(String queueName, String key, String value) throws Exception {
		Jedis jedis = null;
		try {
			jedis = getRedis();
			logger.info("[redis队列] ==> 发送数据" + value + "到" + key + "队列");
			//jedis.sadd(key, value);
			jedis.hset(queueName, key, value);
